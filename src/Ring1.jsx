...
function createShapeGeometry(topCornerRadius, bottomCornerRadius, width, height, ellipseRadius) {
  let actualTopCornerRadius = width * Math.min(topCornerRadius, 100) / 100;
  let actualBottomCornerRadius = height * Math.min(bottomCornerRadius, 100) / 100;

  if (actualTopCornerRadius + actualBottomCornerRadius > Math.min(width, height)) {
    const scaleFactor = Math.min(width, height) / (actualTopCornerRadius + actualBottomCornerRadius);
    actualTopCornerRadius *= scaleFactor;
    actualBottomCornerRadius *= scaleFactor;
  }
